...
const SignIn: React.FC<IProps> = ({ auth, dispatch }) => {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (field.email && field.password && !error.email && !error.password) {
			...
			try {
				setLoading(true);
				await firebase.signin(field.email, field.password);
				setLoading(false);
			} catch(e) {
				setLoading(false);
				// setError({ ...error, auth: e.message });
				dispatch({ type: 'AUTH_FAILED', payload: e.message });
			}
		}
	};
...
export default React.memo(withContext<IProps>(mapState)(SignIn));
